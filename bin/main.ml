    ...
    ; ( "endpoints"
      , `List
          ...
          ; `Assoc
              ...
              ; "description", `String "Parse a Parsecos.Parsed_ast.model JSON payload and return ECOS_BB matrices as JSON"
              ]
          ; `Assoc
              [ "method", `String "GET"
              ; "path", `String "/health"
...
let callback _connection request body =
  ...
  | `POST, "/v1/dsl" ->
    ...
    ecos_response (Parsecos.ecos_data_of_text body)
  | `POST, "/v1/json" ->
    Cohttp_lwt.Body.to_string body >>= fun body ->
    ecos_response (Parsecos.ecos_data_of_json_string body)

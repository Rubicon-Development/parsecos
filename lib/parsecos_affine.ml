...
let of_var var =
  { constant = 0.0; terms = Map.singleton (module Parsecos_var.Scalar) var 1.0 }
;;
...
let term coefficient var =
  ...
  then zero
    ...
    { constant = 0.0; terms = Map.singleton (module Parsecos_var.Scalar) var coefficient }
;;
...
let sub left right =
  add left { constant = -.right.constant; terms = Map.map right.terms ~f:Float.neg }
;;
